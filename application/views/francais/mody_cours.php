...
<div class="comment" style="margin:15px auto auto 20;text-align:left;">
	<?php $last_level = 0; foreach($courses as $cours): 
		...
		<li>
			...
			<a class=cours-frame 
			data-cours_id="<?php echo $cours['cours_id'];?>"
			style="<?php if(!$cours['visible']) echo 'background-color:#cccccc;'?>" 
			href=<?php  echo site_url("/admin/add_cours/"); ?> >
			<?php echo $cours['name']; ?>
			</a>
		</li>
		</br>
	...
	$(document).ready(function(){
		$('.cours-frame').click(function(){
			$.ajax({
			 type: "POST",
			 url: "set_cours_id", 
			 data: {cours_id: $(this).data('cours_id')},
			 dataType: "text",  
			 cache:false,
			 /*success: 
				function(data){
